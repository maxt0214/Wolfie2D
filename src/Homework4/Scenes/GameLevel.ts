...
export default class GameLevel extends Scene {
    ...
    protected initPlayer(): void {
        ...
        this.player.tweens.add("death", {
            ...
            onEnd: HW4_Events.PLAYER_KILLED
        });
    ...
    */
    ...
    protected handlePlayerEnemyCollision(player: AnimatedSprite, enemy: AnimatedSprite) {
        let playerBox = player.boundary;
        let enemyBox = enemy.boundary;
        console.log(playerBox.bottomRight.y - enemyBox.topRight.y);
        if (enemy.imageId == "GhostBunny"){
            if (playerBox.bottomRight.y - enemyBox.topRight.y >= 28){
                Input.disableInput();
                player.disablePhysics();
                this.emitter.fireEvent(GameEventType.PLAY_SOUND, {key: "player_death", loop: false, holdReference: false});
                player.tweens.play("death");
            }
            else{
            ...
            if (enemyBox.bottomRight.y - playerBox.topRight.y >= 16){
                Input.disableInput();
                player.disablePhysics();
                this.emitter.fireEvent(GameEventType.PLAY_SOUND, {key: "player_death", loop: false, holdReference: false});
                player.tweens.play("death");
